...
def run_gui():
  ...
  def make_card(parent, title):
    ...
    tk.Label(inner, text=title, font=("Courier New", 9), fg=MUTED, bg=CARD).pack(anchor="w")
    return inner
  ...
  for var, label, col in [
    ...
    tk.Label(time_row, text=label, font=("Courier New", 9), fg=MUTED, bg=CARD).grid(row=1, column=col, pady=(2, 0))
    if col < 4:
      tk.Label(time_row, text=":", font=("Courier New", 26, "bold"), fg=MUTED, bg=CARD).grid(row=0, column=col + 1)

  presets_row = tk.Frame(time_card, bg=CARD)
  presets_row.pack(fill="x", pady=(12, 0))
  tk.Label(presets_row, text="PRESETS:", font=("Courier New", 9), fg=MUTED, bg=CARD).pack(side="left", padx=(0, 8))
  ...
  def on_close():
    if timer_ref[0] and not timer_ref[0].cancelled:
      if messagebox.askyesno("Timer Running", "A timer is active. Cancel it and exit?"):
        timer_ref[0].cancel()
        root.destroy()
      else:
        root.destroy()
